...
async def joke_writer(req_body: associationsReqBody):
  ...
  while True:
    try: 
      return_state = association_agent.invoke(state)
      break
...
@app.post("/api/joke-writer/joke")
async def write_joke(joke_materials: Dict[str, str]):
  ...
  while retries > 0:
    try: 
      return_state = joke_agent.invoke(state)
      break
    except Exception as e:
      retries -= 1
